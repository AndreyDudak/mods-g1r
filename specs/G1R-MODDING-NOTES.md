...
| `StatEditorMod` | Numpad 0=stats, Numpad 5=stats, Numpad 7=slot lookup, Numpad 8=inventory | Yes (test in-game) |
...
Prototype mod: `Mods/StatEditorMod/` — command line only (UMG crashes on G1R).

| Key | Action |
|---|---|
| **Numpad 0** | Dump all stats to log |
| **Numpad 5** | Stat command line: `lp 100` + Enter, Esc close |
| **Numpad 6** | Dump inventory list with UI `pos` numbers (footer, e.g. 18/334) |
| **Numpad 7** | Lookup: `18` + Enter — item at UI pos 18 |
| **Numpad 8** | Inventory command line: `18 55` or `18 -10` + Enter, Esc close |

| Example | Action |
|---|---|
| Numpad 0 | dump all stats |
| `lp 100` | set SkillPoints |
| `mp 200` | set Mana |
| `mm 250` | set MaxMana |
...
| Key | Action |
|-----|--------|
| **Numpad 6** | Dump all items: `pos 18: Ore x 5  (id 5)` |
...
4. **Keybinds + log** — `StatEditorMod` phase 1 (current)
...
- After editing `.lua` — restart game (hot reload disabled)
